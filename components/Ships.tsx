...
export const ShipCard = ({ship}: {ship: Ship}) => {
  ...
  return (
    <div key={shipSymbol} className="border p-4 m-2">
      ...
      <p className={`text-sm ${transiting && 'text-orange-300'}`}>Location: {ship.nav.route.destination.type}</p>
      <p className='text-sm'>Fuel: <span className={`${
        transiting ? 'animate-pulse' : null
        }`}>
        {displayFuel(ship.fuel.current, ship.fuel.capacity)} {ship.fuel.capacity ? Math.round((100 * ship.fuel.current) / ship.fuel.capacity) : ''}%</span></p>
